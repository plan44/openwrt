...
#define D2_KNOWN(f) cpu_to_le16(IEEE80211_RADIOTAP_HE_DATA2_##f##_KNOWN)
		...
		/* 'pos' carries the sub-object provenance of rtap->it_present
		 * (struct ieee80211_radiotap_header became a __struct_group in
		 * 6.5), which trips FORTIFY's __write_overflow_field check.  The
		 * destination is within the rtap_len-byte buffer pushed onto the
		 * skb, so the write is in fact safe. */
		unsafe_memcpy(pos, &he, sizeof(he),
			      /* dest is within the skb_push'd radiotap buffer */);
#else
