...
platform_do_upgrade() {
	...
	rockchip,rv1106g-luckfox-pico-86panel-w)
	  # set BOOTDEV_MAJOR and BOOTDEV_MINOR env vars
		export_bootdevice
		# set "rootdev" env var (can by any var name as needed)
		...
		CI_ROOTDEV="$rootdev"
		# CI_xx are partition names, known by the kernel via /sys/block/$rootdev/${rootdev}p*/uevent
		...
		#export_partdevice rootfspart 4
...
platform_check_image() {
	...
	case "$(board_name)" in
	rockchip,rv1106g-luckfox-pico-86panel-w)
	  #test for fit image, which is NOT what we have, we use tar
    # [ "$magic" != "d00dfeed" ] && {
